...
Purpose of the micro service: Get active test Count
'''
...
#TEST EVENT
testEvent = {"body":{"organizationId":1,"testMode":1,"userId":"sameer"}}
	...
	if('error' in validRequest):
		...
		if(validRequest["userGroup"] == "admin") :
			...
			if("organizationId" in validRequest):
				...
				data = db_utility.fetch_count_by("test",{"is_active":True,"organization_id":organizationId})
			else :
				data = db_utility.fetch_count_by("test",{"is_active":True})
			if(data != "401") :
				response["activeTestCount"]=data
		else :
