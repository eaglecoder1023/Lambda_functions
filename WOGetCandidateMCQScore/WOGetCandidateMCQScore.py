...
def overallScore(testId):
    return sum(i['score'] if ("score" in i) else 0 for i in db.fetch_where_and(MCQ_SCORE, [{"test_id": testId}]))+sum(i['answer_descriptive_score'] if ("answer_descriptive_score" in i) else 0 for i in db.fetch_where_and(DESC_SCORE, [{"test_id": testId}]))
...
def totalNoOfMCQQuestion(validRequest):
	...
	totalMCQQuestion = test_question_tracker.aggregate_raw_batches([{"$match": {"test_id": validRequest['testId']}}, {"$lookup": {"from": "question_master", "localField": "question_id", "foreignField": "question_id", "as": "question_type"}}, {
	                                                               "$unwind": "$question_type"}, {"$project": {"_id": 0, "question_id": 1, "question_type.question_type_id": 1, "question_type.score": 1}}, {"$match": {"question_type.question_type_id": MCQ_TYPE_ID}}])
	...
	if('error' in validRequest):
		...
		try:
			...
			if(totalMCQQuestions == 0):
				...
				response['overallScore'] = int(overallScore(validRequest['testId']))
			...
			mcqAttended = db.fetch_where_and(
				MCQ_SCORE, [{"test_id": validRequest['testId']}])
